...
namespace KZEngine {
	...
	public:
		//构造函数
		KZCamera(const KZMath::KZVector4D& pos_in = KZMath::KZVector4D(0, 0, 3), float fov = 45.0f, float aspect = (3.0f/2.0f), float near = 1.0f, float far = 20.0f);
		//获取相机变换矩阵
		...
		//返回摄像机位置
		inline KZMath::KZVector4D& GetCameraPos() {
			return pos_;
		}
		//获取摄像机远截面
