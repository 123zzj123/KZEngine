...
KZCamera::KZCamera(const KZMath::KZVector4D& pos, float fov, float aspect, float near, float far) :n_(KZMath::KZVector4D(0, 0, -1, 0)), movement_speed_(kSpeed), mouse_sensity_(kSensitivity),yaw_(kYaw),pitch_(kPitch) {
	...
	near_clip_z_ = near;
	far_clip_z_ = far;
	top_ = near_clip_z_ * tan(KZMath::AngleToRadian(fov * 0.5f));
	bottom_ = -top_;
...
void KZCamera::UpdateCameraVectors() {
	n_.x_ = cos(pitch_) * cos(yaw_);
	n_.y_ = sin(pitch_);
	n_.z_ = cos(pitch_) * sin(yaw_);
	n_.Vector3Normalize();
	n_.Vector3Cross(u_, world_up_);
	u_.Vector3Cross(v_, n_);
}