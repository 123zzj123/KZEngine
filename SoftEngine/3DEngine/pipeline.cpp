...
void KZPipeLine::CreateCube(float width, float length, float height, 
	bool light_cube, const KZMath::KZVector4D<float>& world_pos, const KZMath::KZQuat& quat, const KZMath::KZVector4D<float>& scale, bool is_light, float alpha, string shadow_map) {
	KZObject cube_obj;
	//设置物体id
	...
	if (!is_alpha)
	{
		...
		++object_num_;
		object_vec_.push_back(move(cube_obj));
		//加入活跃物体列表
	...
	else
	{
		...
		++transparent_object_num_;
		transparent_object_vec_.push_back(move(cube_obj));
		sort(transparent_object_vec_.begin(), transparent_object_vec_.end(), [](KZObject a, KZObject b) {return a.world_pos_.z_ > b.world_pos_.z_; });
		//加入活跃物体列表
		transparent_object_active_.push_back(true);
	}
}

//创建三棱锥
void KZPipeLine::CreatePyramid(const KZMath::KZVector4D<float>& world_pos, const KZMath::KZQuat& quat, const KZMath::KZVector4D<float>& scale, bool is_light, float alpha, string shadow_map) {
	KZObject pyramid_obj;
	//设置物体id
	...
	//将顶点加入列表
	pyramid_obj.vlist_local_.push_back(move(v1));
	pyramid_obj.vlist_local_.push_back(move(v2));
	pyramid_obj.vlist_local_.push_back(move(v3));
	pyramid_obj.vlist_local_.push_back(move(v4));
	...
	if (!is_alpha)
	{
		...
		++object_num_;
		object_vec_.push_back(move(pyramid_obj));
		//加入活跃物体列表
		object_active_.push_back(true);
	}
	else
	{
		...
		++transparent_object_num_;
		transparent_object_vec_.push_back(move(pyramid_obj));
		sort(transparent_object_vec_.begin(), transparent_object_vec_.end(), [](KZObject a, KZObject b) {return a.world_pos_.z_ > b.world_pos_.z_; });
		//加入活跃物体列表
		transparent_object_active_.push_back(true);
	}
	...
	string shadow_map)
{
	...
	if (!is_alpha)
	{
		...
		++object_num_;
		object_vec_.push_back(move(cylinder_obj));
		//加入活跃物体列表
		object_active_.push_back(true);
	}
	else
	{
		...
		++transparent_object_num_;
		transparent_object_vec_.push_back(move(cylinder_obj));
		sort(transparent_object_vec_.begin(), transparent_object_vec_.end(), [](KZObject a, KZObject b) {return a.world_pos_.z_ > b.world_pos_.z_; });
		//加入活跃物体列表
		transparent_object_active_.push_back(true);
	}
	...
	string shadow_map)
{
	...
	if (!is_alpha)
	{
		...
		++object_num_;
		object_vec_.push_back(move(sphere_obj));
		//加入活跃物体列表
		object_active_.push_back(true);
	}
	else
	{
		...
		++transparent_object_num_;
		transparent_object_vec_.push_back(move(sphere_obj));
		sort(transparent_object_vec_.begin(), transparent_object_vec_.end(), [](KZObject a, KZObject b) {return a.world_pos_.z_ > b.world_pos_.z_; });
		//加入活跃物体列表
		transparent_object_active_.push_back(true);
	}
	...
	const KZEngine::Color& ini_color, bool is_light, float alpha, const KZMath::KZVector4D<float>& world_pos, const KZMath::KZQuat& quat)
{
	...
	if (h_img_height != 0 && h_img_width != 0)
	{
		...
		{
			++object_num_;
			object_vec_.push_back(move(terrain_obj));
			//加入活跃物体列表
			object_active_.push_back(true);
		}
		else
		{
			...
			++transparent_object_num_;
			transparent_object_vec_.push_back(move(terrain_obj));
			sort(transparent_object_vec_.begin(), transparent_object_vec_.end(), [](KZObject a, KZObject b) {return a.world_pos_.z_ > b.world_pos_.z_; });
			//加入活跃物体列表
			transparent_object_active_.push_back(true);
		}
...
void KZPipeLine::addShadowObj(const string& shadow_name, const string& shadow_map_texture, const KZMath::KZVector4D<float>& ori_world_pos, const float radius)
{
	...
	//将顶点加入列表
	shadow_obj.vlist_local_.push_back(move(v1));
	shadow_obj.vlist_local_.push_back(move(v2));
	shadow_obj.vlist_local_.push_back(move(v3));
	shadow_obj.vlist_local_.push_back(move(v4));
	...
	//阴影物体视为透明物体
	++transparent_object_num_;
	transparent_object_vec_.push_back(move(shadow_obj));
	sort(transparent_object_vec_.begin(), transparent_object_vec_.end(), [](KZObject a, KZObject b) {return a.world_pos_.z_ > b.world_pos_.z_; });
	//加入活跃物体列表
	transparent_object_active_.push_back(true);
}
...
void KZPipeLine::OcclusionCulling() {
	...
	for (uint32_t i = 0; i < object_num_; ++i) {
		temp_camera_pos = view * object_vec_[i].world_pos_;
		//根据远近平面裁剪
		if ((temp_camera_pos.z_ + object_vec_[i].max_radius_ < main_camera_.GetCameraFarClip()) || (temp_camera_pos.z_ - object_vec_[i].max_radius_ > main_camera_.GetCameraNearClip())) {
			object_active_[i] = false;
			continue;
		}
		//根据左右平面裁剪
		float w_test = camera_right_over_near * temp_camera_pos.z_;
		if ((temp_camera_pos.x_ - object_vec_[i].max_radius_ > w_test) || (temp_camera_pos.x_ + object_vec_[i].max_radius_ < -w_test)) {
			object_active_[i] = false;
			continue;
		}
		...
		if ((temp_camera_pos.y_ - object_vec_[i].max_radius_ > h_test) || (temp_camera_pos.y_ + object_vec_[i].max_radius_ < -h_test)) {
			object_active_[i] = false;
			continue;
		}
	}
	...
	//透明物体
	for (uint32_t i = 0; i < transparent_object_num_; ++i) {
		temp_camera_pos = view * object_vec_[i].world_pos_;
		//根据远近平面裁剪
		if ((temp_camera_pos.z_ - transparent_object_vec_[i].max_radius_ < main_camera_.GetCameraFarClip()) || (temp_camera_pos.z_ + transparent_object_vec_[i].max_radius_ > main_camera_.GetCameraNearClip())) {
			transparent_object_active_[i] = false;
			continue;
		}
		//根据左右平面裁剪
		float w_test = camera_right_over_near * temp_camera_pos.z_;
		if ((temp_camera_pos.x_ + transparent_object_vec_[i].max_radius_ > w_test) || (temp_camera_pos.x_ - transparent_object_vec_[i].max_radius_ < -w_test)) {
			transparent_object_active_[i] = false;
			continue;
		}
		...
		//根据上下平面裁剪
		float h_test = camera_top_over_near * temp_camera_pos.z_;
		if ((temp_camera_pos.y_ + transparent_object_vec_[i].max_radius_ > h_test) || (temp_camera_pos.y_ - transparent_object_vec_[i].max_radius_ < -h_test)) {
			transparent_object_active_[i] = false;
			continue;
		}
	}
...
void KZPipeLine::BackfaceCulling() {
	...
	for (uint32_t i = 0; i < object_num_; ++i) {
		if (object_active_[i]) {
			...
			for (uint32_t j = 0; j < object_vec_[i].num_index_; face_index++, j += 3) {
				KZMath::KZVector4D<float> observe_vec = main_camera_.GetCameraPos() - object_vec_[i].vlist_tran_[object_vec_[i].index_[j]].pos;
					...
					if (object_vec_[i].is_light_) {
						...
						for (uint32_t k = 0; k < light_num; ++k) {
							if (light_active_vec_[k]) {
								object_vec_[i].vlist_tran_[object_vec_[i].index_[idx0]].color += mat_vec_[object_vec_[i].mat_id_[face_index]].CalculateFinalColor(light_vec_[k],
									object_vec_[i].vlist_tran_[object_vec_[i].index_[idx0]].pos,
									object_vec_[i].vlist_tran_[object_vec_[i].index_[idx0]].normal);
								object_vec_[i].vlist_tran_[object_vec_[i].index_[idx1]].color += mat_vec_[object_vec_[i].mat_id_[face_index]].CalculateFinalColor(light_vec_[k],
									object_vec_[i].vlist_tran_[object_vec_[i].index_[idx1]].pos,
									object_vec_[i].vlist_tran_[object_vec_[i].index_[idx1]].normal);
								object_vec_[i].vlist_tran_[object_vec_[i].index_[idx2]].color += mat_vec_[object_vec_[i].mat_id_[face_index]].CalculateFinalColor(light_vec_[k],
									object_vec_[i].vlist_tran_[object_vec_[i].index_[idx2]].pos,
									object_vec_[i].vlist_tran_[object_vec_[i].index_[idx2]].normal);
							}
						}
					}
					tri.vertex_list[0] = object_vec_[i].vlist_tran_[object_vec_[i].index_[idx0]];
					tri.vertex_list[1] = object_vec_[i].vlist_tran_[object_vec_[i].index_[idx1]];
					tri.vertex_list[2] = object_vec_[i].vlist_tran_[object_vec_[i].index_[idx2]];
					tri.material = object_vec_[i].mat_id_[face_index];
					++tri_num_;
					if (tri_num_ > render_list_->tri_list_.size()) {
						render_list_->tri_list_.resize(tri_num_ * 2);
						render_list_->active_tri_.resize(tri_num_ * 2);
					}
					render_list_->tri_list_[tri_num_ - 1] = tri;
					render_list_->active_tri_[tri_num_ - 1] = true;
				}
			}
		}
	}
	...
	//透明物体
	for (uint32_t i = 0; i < transparent_object_num_; ++i) {
		if (transparent_object_active_[i]) {
			uint32_t face_index = 0;
			for (uint32_t j = 0; j < transparent_object_vec_[i].num_index_; face_index++, j += 3) {
				KZMath::KZVector4D<float> observe_vec = main_camera_.GetCameraPos() - transparent_object_vec_[i].vlist_tran_[transparent_object_vec_[i].index_[j]].pos;
				if (transparent_object_vec_[i].face_normal_[face_index].Vector3Dot(observe_vec) < 0) {
					continue;
				}
				else
				{
					KZEngine::Triangle tri;
					uint32_t idx0 = j, idx1 = j + 1, idx2 = j + 2;
					if (transparent_object_vec_[i].is_light_) {
						uint32_t light_num = static_cast<uint32_t>(light_vec_.size());
						for (uint32_t k = 0; k < light_num; ++k) {
							if (light_active_vec_[k]) {
								transparent_object_vec_[i].vlist_tran_[transparent_object_vec_[i].index_[idx0]].color += mat_vec_[transparent_object_vec_[i].mat_id_[face_index]].CalculateFinalColor(light_vec_[k],
									transparent_object_vec_[i].vlist_tran_[transparent_object_vec_[i].index_[idx0]].pos,
									transparent_object_vec_[i].vlist_tran_[transparent_object_vec_[i].index_[idx0]].normal);
								transparent_object_vec_[i].vlist_tran_[transparent_object_vec_[i].index_[idx1]].color += mat_vec_[transparent_object_vec_[i].mat_id_[face_index]].CalculateFinalColor(light_vec_[k],
									transparent_object_vec_[i].vlist_tran_[transparent_object_vec_[i].index_[idx1]].pos,
									transparent_object_vec_[i].vlist_tran_[transparent_object_vec_[i].index_[idx1]].normal);
								transparent_object_vec_[i].vlist_tran_[transparent_object_vec_[i].index_[idx2]].color += mat_vec_[transparent_object_vec_[i].mat_id_[face_index]].CalculateFinalColor(light_vec_[k],
									transparent_object_vec_[i].vlist_tran_[transparent_object_vec_[i].index_[idx2]].pos,
									transparent_object_vec_[i].vlist_tran_[transparent_object_vec_[i].index_[idx2]].normal);
							}
						}
					}
					tri.vertex_list[0] = transparent_object_vec_[i].vlist_tran_[transparent_object_vec_[i].index_[idx0]];
					tri.vertex_list[1] = transparent_object_vec_[i].vlist_tran_[transparent_object_vec_[i].index_[idx1]];
					tri.vertex_list[2] = transparent_object_vec_[i].vlist_tran_[transparent_object_vec_[i].index_[idx2]];
					tri.alpha = transparent_object_vec_[i].alpha_;
					tri.material = transparent_object_vec_[i].mat_id_[face_index];
					++transparent_tri_num_;
					if (transparent_tri_num_ > render_list_->transparent_tri_list_.size()) {
						uint32_t new_size = transparent_tri_num_ << 1; // tri_num_ * 2
						render_list_->transparent_tri_list_.resize(new_size);
						render_list_->transparent_active_tri_.resize(new_size);
					}
					render_list_->transparent_tri_list_[transparent_tri_num_ - 1] = tri;
					render_list_->transparent_active_tri_[transparent_tri_num_ - 1] = true;
				}
			}
		}
	}
...
void KZPipeLine::TransformModelToWorld() {
	for (uint32_t i = 0; i < object_num_; ++i) {
		...
		if (calculate_shadow_ == CalCulateShadow::VERTEXMAPPING && object_vec_[i].has_shadow && object_vec_[i].world_pos_.y_ > 0)
		{
			KZMath::KZVector4D<float> light_pos;
			light_vec_[0]->GetLightPos(light_pos);
			...
			{
				//经过点光源与物体中心直线与地面相交参数方程 ps = pl + t * (p0 - pl), 令ps.y = 0;
				float t = -light_pos.y_ / (object_vec_[i].vlist_tran_[j].pos.y_ - light_pos.y_);
				float temp_x = light_pos.x_ + t * (object_vec_[i].vlist_tran_[j].pos.x_ - light_pos.x_);
				float temp_z = light_pos.z_ + t * (object_vec_[i].vlist_tran_[j].pos.z_ - light_pos.z_);
				object_vec_[i].vlist_tran_.push_back(KZEngine::Vertex(temp_x, 0.0f, temp_z, 128, 128, 128));
			}
		}
	}
	for (uint32_t i = 0; i < transparent_object_num_; ++i)
	{
		transparent_object_vec_[i].TransformModelToWorldMath();
	}
}
...
void KZPipeLine::TransformWorldToPer(Projection projection) {
	...
	for (uint32_t i = 0; i < tri_num_; ++i) {
		for (uint32_t j = 0; j < 3; ++j) {
			render_list_->tri_list_[i].vertex_list[j].pos = view * render_list_->tri_list_[i].vertex_list[j].pos;
		}
	}
	for (uint32_t i = 0; i < transparent_tri_num_; ++i)
	{
		for (uint32_t j = 0; j < 3; ++j)
		{
			render_list_->transparent_tri_list_[i].vertex_list[j].pos = view * render_list_->transparent_tri_list_[i].vertex_list[j].pos;
		}
	}
	PolyCulling();
	for (uint32_t i = 0; i < tri_num_; ++i) {
		if (render_list_->active_tri_[i]) {
			for (uint32_t j = 0; j < 3; ++j) {
				render_list_->tri_list_[i].vertex_list[j].pos = proj * render_list_->tri_list_[i].vertex_list[j].pos;
	...
	for (uint32_t i = 0; i < transparent_tri_num_; ++i)
	{
		if (render_list_->transparent_active_tri_[i])
		{
			for (uint32_t j = 0; j < 3; ++j)
			{
				render_list_->transparent_tri_list_[i].vertex_list[j].pos = proj * render_list_->transparent_tri_list_[i].vertex_list[j].pos;
				float w_inverse = 1 / render_list_->transparent_tri_list_[i].vertex_list[j].pos.w_;
				render_list_->transparent_tri_list_[i].vertex_list[j].pos.x_ *= w_inverse;
				render_list_->transparent_tri_list_[i].vertex_list[j].pos.y_ *= w_inverse;
				render_list_->transparent_tri_list_[i].vertex_list[j].pos.z_ *= w_inverse;
			}
		}
	}
}
...
void KZPipeLine::TransformPerToViewPort() {
	...
	for (uint32_t i = 0; i < transparent_tri_num_; ++i) {
		if (render_list_->transparent_active_tri_[i]) {
			for (uint32_t j = 0; j < 3; ++j) {
				render_list_->transparent_tri_list_[i].vertex_list[j].pos.x_ = alpha + alpha * render_list_->transparent_tri_list_[i].vertex_list[j].pos.x_;
				render_list_->transparent_tri_list_[i].vertex_list[j].pos.y_ = beta - beta * render_list_->transparent_tri_list_[i].vertex_list[j].pos.y_;
			}
		}
	}
}
...
void KZPipeLine::FrameUpdate() {
	...
	////初始化
	tri_num_ = 0;
	transparent_tri_num_ = 0;
	fill_n(object_active_.begin(), object_num_, true);
	fill_n(transparent_object_active_.begin(), transparent_object_num_, true);
...
void KZPipeLine::RasterizationDepthTest() {
	for (uint32_t i = 0; i < tri_num_; ++i) {
		...
		if (render_list_->active_tri_[i]) {
			...
			if (floor_v1_y == floor_v2_y) {
				...
				DrawBottomTri(v0, v1, v2, has_texture, render_list_->tri_list_[i].material, render_list_->tri_list_[i].alpha);
			}
			//检查是否平顶三角形
				...
				//thread_top.detach();
				DrawTopTri(v0, v1, v2, has_texture, render_list_->tri_list_[i].material, render_list_->tri_list_[i].alpha);
			}
			//一般三角形先做分割
			else
			{
				...
				{
					DrawBottomTri(v0, v1, new_vertex, has_texture, render_list_->tri_list_[i].material, render_list_->tri_list_[i].alpha);
				}
				if (new_vertex.pos.y_ <= view_height_)
				{
					DrawTopTri(new_vertex, v1, v2, has_texture, render_list_->tri_list_[i].material, render_list_->tri_list_[i].alpha);
				}
			}
		}
	}
	for (uint32_t i = 0; i < transparent_tri_num_; ++i) {
		...
		if (render_list_->transparent_active_tri_[i]) {
			Vertex temp_v(0, 0, 0);
			Vertex v0 = render_list_->transparent_tri_list_[i].vertex_list[0];
			Vertex v1 = render_list_->transparent_tri_list_[i].vertex_list[1];
			Vertex v2 = render_list_->transparent_tri_list_[i].vertex_list[2];
			float dx_left = 0.0f, dx_right = 0.0f, dz_left = 0.0f, dz_right = 0.0f;
			...
			bool has_texture = false;
			if (render_list_->transparent_tri_list_[i].material != -1 && mat_vec_[render_list_->transparent_tri_list_[i].material].has_texture_) {
				has_texture = true;
			}
			...
			if (floor_v1_y == floor_v2_y) {
				...
				//std::thread thread_bottom(&KZPipeLine::DrawBottomTri, KZPipeLine::GetInstance(), v0, v1, v2, has_texture, render_list_->tri_list_[i].material);
				//thread_bottom.detach();
				DrawBottomTri(v0, v1, v2, has_texture, render_list_->transparent_tri_list_[i].material, render_list_->transparent_tri_list_[i].alpha);
			}
			//检查是否平顶三角形
			else if (floor_v0_y == floor_v1_y) {
				//std::thread thread_top(&KZPipeLine::DrawTopTri, KZPipeLine::GetInstance(), v0, v1, v2, has_texture, render_list_->tri_list_[i].material);
				//thread_top.detach();
				DrawTopTri(v0, v1, v2, has_texture, render_list_->transparent_tri_list_[i].material, render_list_->transparent_tri_list_[i].alpha);
			}
			//一般三角形先做分割
			else
			{
				...
				new_vertex.uv.x_ = new_u;
				new_vertex.uv.y_ = new_v;
				new_vertex.normal = render_list_->transparent_tri_list_[i].face_normal;
				//将一般三角形分为平顶三角形和平底三角形
				//std::thread thread_bottom(&KZPipeLine::DrawBottomTri, KZPipeLine::GetInstance(), v0, v1, new_vertex, has_texture, render_list_->tri_list_[i].material);
				//thread_bottom.detach();
				//std::thread thread_top(&KZPipeLine::DrawTopTri, KZPipeLine::GetInstance(), new_vertex, v1, v2, has_texture, render_list_->tri_list_[i].material);
				...
				if (new_vertex.pos.y_ >= 0)
				{
					DrawBottomTri(v0, v1, new_vertex, has_texture, render_list_->transparent_tri_list_[i].material, render_list_->transparent_tri_list_[i].alpha);
				}
				if (new_vertex.pos.y_ <= view_height_)
				{
					DrawTopTri(new_vertex, v1, v2, has_texture, render_list_->transparent_tri_list_[i].material, render_list_->transparent_tri_list_[i].alpha);
				}
			}
		}
	}
...
void KZPipeLine::RasterizationDepthTestFast() {
	for (uint32_t i = 0; i < tri_num_; ++i) {
		...
		if (render_list_->active_tri_[i]) {
			Vertex temp_v(0, 0, 0);
			Vertex vertex0 = render_list_->tri_list_[i].vertex_list[0];
			Vertex vertex1 = render_list_->tri_list_[i].vertex_list[1];
			Vertex vertex2 = render_list_->tri_list_[i].vertex_list[2];
			float dx_left = 0.0f, dx_right = 0.0f, dz_left = 0.0f, dz_right = 0.0f;
			...
			//检查是否平底三角形
			if (floor_v1_y == floor_v2_y) {
				DrawBottomTriFast(vertex0, vertex1, vertex2, has_texture, render_list_->tri_list_[i].material);
			}
			//检查是否平顶三角形
			else if (floor_v0_y == floor_v1_y) {
				DrawTopTriFast(vertex0, vertex1, vertex2, has_texture, render_list_->tri_list_[i].material);
			}
			//一般三角形先做分割
			else
			{
				...
				new_vertex.uv.y_ = new_v;
				new_vertex.normal = render_list_->tri_list_[i].face_normal;
				DrawBottomTriFast(vertex0, vertex1, new_vertex, has_texture, render_list_->tri_list_[i].material);
				DrawTopTriFast(new_vertex, vertex1, vertex2, has_texture, render_list_->tri_list_[i].material);
			}
		}
	}
	for (uint32_t i = 0; i < transparent_tri_num_; ++i) {
		...
		if (render_list_->transparent_active_tri_[i]) {
			Vertex temp_v(0, 0, 0);
			Vertex vertex0 = render_list_->transparent_tri_list_[i].vertex_list[0];
			Vertex vertex1 = render_list_->transparent_tri_list_[i].vertex_list[1];
			Vertex vertex2 = render_list_->transparent_tri_list_[i].vertex_list[2];
			float dx_left = 0.0f, dx_right = 0.0f, dz_left = 0.0f, dz_right = 0.0f;
			...
			//检查是否平底三角形
			if (floor_v1_y == floor_v2_y) {
				DrawBottomTriFast(vertex0, vertex1, vertex2, has_texture, render_list_->transparent_tri_list_[i].material, render_list_->transparent_tri_list_[i].alpha);
			}
			//检查是否平顶三角形
			else if (floor_v0_y == floor_v1_y) {
				DrawTopTriFast(vertex0, vertex1, vertex2, has_texture, render_list_->transparent_tri_list_[i].material, render_list_->transparent_tri_list_[i].alpha);
			}
			//一般三角形先做分割
			else
			{
				...
				new_vertex.uv.y_ = new_v;
				new_vertex.normal = render_list_->transparent_tri_list_[i].face_normal;
				DrawBottomTriFast(vertex0, vertex1, new_vertex, has_texture, render_list_->tri_list_[i].material, render_list_->transparent_tri_list_[i].alpha);
				DrawTopTriFast(new_vertex, vertex1, vertex2, has_texture, render_list_->tri_list_[i].material, render_list_->transparent_tri_list_[i].alpha);
			}
		}
	}
}
...
void KZPipeLine::PolyCulling() {
	...
	for (uint32_t i = 0; i < tri_num; ++i) {
		//考虑左右截面的裁剪,利用三角形相似
		float x0_test = w_factor * render_list_->tri_list_[i].vertex_list[0].pos.z_;
		float x1_test = w_factor * render_list_->tri_list_[i].vertex_list[1].pos.z_;
		float x2_test = w_factor * render_list_->tri_list_[i].vertex_list[2].pos.z_;
		//对于左右截面只考虑完全接受或者完全拒绝，即所有点都在视锥体右边或者都在视锥体左边
		if ((render_list_->tri_list_[i].vertex_list[0].pos.x_ > x0_test
			&& render_list_->tri_list_[i].vertex_list[1].pos.x_ > x1_test
			&& render_list_->tri_list_[i].vertex_list[2].pos.x_ > x2_test)
			|| (render_list_->tri_list_[i].vertex_list[0].pos.x_ < -x0_test
				&& render_list_->tri_list_[i].vertex_list[1].pos.x_ < -x1_test
				&& render_list_->tri_list_[i].vertex_list[2].pos.x_ < -x2_test))
		{
			render_list_->active_tri_[i] = false;
			continue;
		}

		//考虑上下截面的裁剪，利用三角形相似
		float y0_test = h_factor * render_list_->tri_list_[i].vertex_list[0].pos.z_;
		float y1_test = h_factor * render_list_->tri_list_[i].vertex_list[1].pos.z_;
		float y2_test = h_factor * render_list_->tri_list_[i].vertex_list[2].pos.z_;
		//对于上下截面只考虑完全接受或者完全拒绝，即所有点都在视锥体上边或者都在视锥体下边
		if ((render_list_->tri_list_[i].vertex_list[0].pos.y_ > y0_test
			&& render_list_->tri_list_[i].vertex_list[1].pos.y_ > y1_test
			&& render_list_->tri_list_[i].vertex_list[2].pos.y_ > y2_test)
			|| (render_list_->tri_list_[i].vertex_list[0].pos.y_ < -y0_test
				&& render_list_->tri_list_[i].vertex_list[1].pos.y_ < -y1_test
				&& render_list_->tri_list_[i].vertex_list[2].pos.y_ < -y2_test))
		{
			render_list_->active_tri_[i] = false;
		...
		if ((render_list_->tri_list_[i].vertex_list[0].pos.z_ < near_clip 
			&& render_list_->tri_list_[i].vertex_list[0].pos.z_ > far_clip)) {
			verts_inside[0] = true;
			++num_verts_inside;
		}
		else if(render_list_->tri_list_[i].vertex_list[0].pos.z_ > near_clip)
		{
			verts_outside[0] = true;
			++num_verts_outside_near_clip;
		}

		if ((render_list_->tri_list_[i].vertex_list[1].pos.z_ < near_clip
			&& render_list_->tri_list_[i].vertex_list[1].pos.z_ > far_clip)) {
			verts_inside[1] = true;
			++num_verts_inside;
		}
		else if(render_list_->tri_list_[i].vertex_list[1].pos.z_ > near_clip)
		{
			verts_outside[1] = true;
			++num_verts_outside_near_clip;
		}

		if ((render_list_->tri_list_[i].vertex_list[2].pos.z_ < near_clip
			&& render_list_->tri_list_[i].vertex_list[2].pos.z_ > far_clip)) {
			verts_inside[2] = true;
			++num_verts_inside;
		}
		else if(render_list_->tri_list_[i].vertex_list[2].pos.z_ > near_clip)
		{
		...
		if (num_verts_outside_near_clip >= 1) {
			...
			if (num_verts_inside == 1)
			{
				...
				float diff = (near_clip - render_list_->tri_list_[i].vertex_list[v0_idx].pos.z_);
				KZMath::KZVector4D<float> vec1(render_list_->tri_list_[i].vertex_list[v1_idx].pos - render_list_->tri_list_[i].vertex_list[v0_idx].pos);
				float t1 = diff / vec1.z_;

				//根据参数t计算出交点的pos,代替原来顶点的pos
				render_list_->tri_list_[i].vertex_list[v1_idx].pos.x_ = render_list_->tri_list_[i].vertex_list[v0_idx].pos.x_ + t1 * vec1.x_;
				render_list_->tri_list_[i].vertex_list[v1_idx].pos.y_ = render_list_->tri_list_[i].vertex_list[v0_idx].pos.y_ + t1 * vec1.y_;
				render_list_->tri_list_[i].vertex_list[v1_idx].pos.z_ = near_clip;

				//根据参数t计算出交点的color,代替原来顶点的color
				render_list_->tri_list_[i].vertex_list[v1_idx].color.r_ = (unsigned char)(render_list_->tri_list_[i].vertex_list[v0_idx].color.r_
					+ t1 * (render_list_->tri_list_[i].vertex_list[v1_idx].color.r_ - render_list_->tri_list_[i].vertex_list[v0_idx].color.r_));

				render_list_->tri_list_[i].vertex_list[v1_idx].color.g_ = (unsigned char)(render_list_->tri_list_[i].vertex_list[v0_idx].color.g_
					+ t1 * (render_list_->tri_list_[i].vertex_list[v1_idx].color.g_ - render_list_->tri_list_[i].vertex_list[v0_idx].color.g_));

				render_list_->tri_list_[i].vertex_list[v1_idx].color.b_ = (unsigned char)(render_list_->tri_list_[i].vertex_list[v0_idx].color.b_
					+ t1 * (render_list_->tri_list_[i].vertex_list[v1_idx].color.b_ - render_list_->tri_list_[i].vertex_list[v0_idx].color.b_));

				//根据线段参数方程p = v0 + v * t (0 < t < 1)
				KZMath::KZVector4D<float> vec2(render_list_->tri_list_[i].vertex_list[v2_idx].pos - render_list_->tri_list_[i].vertex_list[v0_idx].pos);
				float t2 = diff / vec2.z_;
				//根据参数t计算出交点pos，代替原来pos
				render_list_->tri_list_[i].vertex_list[v2_idx].pos.x_ = render_list_->tri_list_[i].vertex_list[v0_idx].pos.x_ + t2 * vec2.x_;
				render_list_->tri_list_[i].vertex_list[v2_idx].pos.y_ = render_list_->tri_list_[i].vertex_list[v0_idx].pos.y_ + t2 * vec2.y_;
				render_list_->tri_list_[i].vertex_list[v2_idx].pos.z_ = near_clip;

				//根据参数t计算出交点的color,代替原来顶点的color
				render_list_->tri_list_[i].vertex_list[v2_idx].color.r_ = (unsigned char)(render_list_->tri_list_[i].vertex_list[v0_idx].color.r_
					+ t2 * (render_list_->tri_list_[i].vertex_list[v2_idx].color.r_ - render_list_->tri_list_[i].vertex_list[v0_idx].color.r_));

				render_list_->tri_list_[i].vertex_list[v2_idx].color.g_ = (unsigned char)(render_list_->tri_list_[i].vertex_list[v0_idx].color.g_
					+ t2 * (render_list_->tri_list_[i].vertex_list[v2_idx].color.g_ - render_list_->tri_list_[i].vertex_list[v0_idx].color.g_));

				render_list_->tri_list_[i].vertex_list[v2_idx].color.b_ = (unsigned char)(render_list_->tri_list_[i].vertex_list[v0_idx].color.b_
					+ t2 * (render_list_->tri_list_[i].vertex_list[v2_idx].color.b_ - render_list_->tri_list_[i].vertex_list[v0_idx].color.b_));
				...
				if (render_list_->tri_list_[i].material != -1 && mat_vec_[render_list_->tri_list_[i].material].has_texture_) {
					...
					render_list_->tri_list_[i].vertex_list[v1_idx].uv.x_ = render_list_->tri_list_[i].vertex_list[v0_idx].uv.x_
						+ t1 * (render_list_->tri_list_[i].vertex_list[v1_idx].uv.x_ - render_list_->tri_list_[i].vertex_list[v0_idx].uv.x_);

					render_list_->tri_list_[i].vertex_list[v1_idx].uv.y_ = render_list_->tri_list_[i].vertex_list[v0_idx].uv.y_
						+ t1 * (render_list_->tri_list_[i].vertex_list[v1_idx].uv.y_ - render_list_->tri_list_[i].vertex_list[v0_idx].uv.y_);

					render_list_->tri_list_[i].vertex_list[v2_idx].uv.x_ = render_list_->tri_list_[i].vertex_list[v0_idx].uv.x_
						+ t2 * (render_list_->tri_list_[i].vertex_list[v2_idx].uv.x_ - render_list_->tri_list_[i].vertex_list[v0_idx].uv.x_);

					render_list_->tri_list_[i].vertex_list[v2_idx].uv.y_ = render_list_->tri_list_[i].vertex_list[v0_idx].uv.y_
						+ t2 * (render_list_->tri_list_[i].vertex_list[v2_idx].uv.y_ - render_list_->tri_list_[i].vertex_list[v0_idx].uv.y_);
				}
			}
			...
			else if (num_verts_inside == 2 && num_verts_outside_near_clip == 1)
			{
				...
				Triangle new_tri(render_list_->tri_list_[i]);
				...
				KZMath::KZVector4D<float> ori_v0_pos = render_list_->tri_list_[i].vertex_list[v0_idx].pos;
				//根据参数t计算出交点的pos,代替外部顶点的pos
				render_list_->tri_list_[i].vertex_list[v0_idx].pos.x_ = render_list_->tri_list_[i].vertex_list[v0_idx].pos.x_ + t1 * vec1.x_;
				render_list_->tri_list_[i].vertex_list[v0_idx].pos.y_ = render_list_->tri_list_[i].vertex_list[v0_idx].pos.y_ + t1 * vec1.y_;
				render_list_->tri_list_[i].vertex_list[v0_idx].pos.z_ = near_clip;

				//根据参数t计算出交点的color,代替外部顶点的color
				float ori_v0_r = render_list_->tri_list_[i].vertex_list[v0_idx].color.r_;
				float ori_v0_g = render_list_->tri_list_[i].vertex_list[v0_idx].color.g_;
				float ori_v0_b = render_list_->tri_list_[i].vertex_list[v0_idx].color.b_;
				render_list_->tri_list_[i].vertex_list[v0_idx].color.r_ = (unsigned char)(render_list_->tri_list_[i].vertex_list[v0_idx].color.r_
					+ t1 * (render_list_->tri_list_[i].vertex_list[v1_idx].color.r_ - render_list_->tri_list_[i].vertex_list[v0_idx].color.r_));
				render_list_->tri_list_[i].vertex_list[v0_idx].color.g_ = (unsigned char)(render_list_->tri_list_[i].vertex_list[v0_idx].color.g_
					+ t1 * (render_list_->tri_list_[i].vertex_list[v1_idx].color.g_ - render_list_->tri_list_[i].vertex_list[v0_idx].color.g_));
				render_list_->tri_list_[i].vertex_list[v0_idx].color.b_ = (unsigned char)(render_list_->tri_list_[i].vertex_list[v0_idx].color.b_
					+ t1 * (render_list_->tri_list_[i].vertex_list[v1_idx].color.b_ - render_list_->tri_list_[i].vertex_list[v0_idx].color.b_));

				//默认法线不变，点法线插值计算不值得特别精确用原本点法线代替即可,并且已经计算了光照

				float ori_v0_u = render_list_->tri_list_[i].vertex_list[v0_idx].uv.x_;
				float ori_v0_v = render_list_->tri_list_[i].vertex_list[v0_idx].uv.y_;
				//根据是否有贴图计算交点的uv,代替原来的uv
				if (render_list_->tri_list_[i].material != -1 && mat_vec_[render_list_->tri_list_[i].material].has_texture_)
				{
					render_list_->tri_list_[i].vertex_list[v0_idx].uv.x_ = render_list_->tri_list_[i].vertex_list[v0_idx].uv.x_
						+ t1 * (render_list_->tri_list_[i].vertex_list[v1_idx].uv.x_ - render_list_->tri_list_[i].vertex_list[v0_idx].uv.x_);

					render_list_->tri_list_[i].vertex_list[v0_idx].uv.y_ = render_list_->tri_list_[i].vertex_list[v0_idx].uv.y_
						+ t1 * (render_list_->tri_list_[i].vertex_list[v1_idx].uv.y_ - render_list_->tri_list_[i].vertex_list[v0_idx].uv.y_);
				}

				//新三角形的保留v2不变
				new_tri.vertex_list[v1_idx] = render_list_->tri_list_[i].vertex_list[v0_idx];

				//根据线段参数方程p = v0 + v * t (0 < t < 1)
				//diff = (near_clip - render_list_->tri_list_[i].vertex_list[v0_idx].pos.z_);
				KZMath::KZVector4D<float> vec2(render_list_->tri_list_[i].vertex_list[v2_idx].pos - ori_v0_pos);
				float t2 = diff / vec2.z_;
				...
				new_tri.vertex_list[v0_idx].color.r_ = (unsigned char)(ori_v0_r +
					t2 * (render_list_->tri_list_[i].vertex_list[v2_idx].color.r_ - ori_v0_r));
				new_tri.vertex_list[v0_idx].color.g_ = (unsigned char)(ori_v0_g +
					t2 * (render_list_->tri_list_[i].vertex_list[v2_idx].color.g_ - ori_v0_g));
				new_tri.vertex_list[v0_idx].color.b_ = (unsigned char)(ori_v0_b +
					t2 * (render_list_->tri_list_[i].vertex_list[v2_idx].color.b_ - ori_v0_b));

				//默认法线不变，点法线插值计算不值得特别精确用原本点法线代替即可,并且已经计算了光照
				new_tri.vertex_list[1].normal = render_list_->tri_list_[i].vertex_list[v2_idx].normal;

				//根据是否有贴图计算交点的uv,代替原来的uv
				if (render_list_->tri_list_[i].material != -1 && mat_vec_[render_list_->tri_list_[i].material].has_texture_)
				{
					new_tri.vertex_list[v0_idx].uv.x_ = ori_v0_u
						+ t2 * (render_list_->tri_list_[i].vertex_list[v2_idx].uv.x_ - ori_v0_u);

					new_tri.vertex_list[v0_idx].uv.y_ = ori_v0_v
						+ t2 * (render_list_->tri_list_[i].vertex_list[v2_idx].uv.y_ - ori_v0_v);
				}
				...
				render_list_->tri_list_[tri_num_ - 1] = new_tri;
				render_list_->active_tri_[tri_num_ - 1] = true;
	...
	for (uint32_t i = 0; i < transparent_tri_num_; ++i) {
		//考虑左右截面的裁剪,利用三角形相似
		float x0_test = w_factor * render_list_->transparent_tri_list_[i].vertex_list[0].pos.z_;
		float x1_test = w_factor * render_list_->transparent_tri_list_[i].vertex_list[1].pos.z_;
		float x2_test = w_factor * render_list_->transparent_tri_list_[i].vertex_list[2].pos.z_;
		//对于左右截面只考虑完全接受或者完全拒绝，即所有点都在视锥体右边或者都在视锥体左边
		if ((render_list_->transparent_tri_list_[i].vertex_list[0].pos.x_ > x0_test
			&& render_list_->transparent_tri_list_[i].vertex_list[1].pos.x_ > x1_test
			&& render_list_->transparent_tri_list_[i].vertex_list[2].pos.x_ > x2_test)
			|| (render_list_->transparent_tri_list_[i].vertex_list[0].pos.x_ < -x0_test
				&& render_list_->transparent_tri_list_[i].vertex_list[1].pos.x_ < -x1_test
				&& render_list_->transparent_tri_list_[i].vertex_list[2].pos.x_ < -x2_test))
		{
			render_list_->transparent_active_tri_[i] = false;
			continue;
		}

		//考虑上下截面的裁剪，利用三角形相似
		float y0_test = h_factor * render_list_->transparent_tri_list_[i].vertex_list[0].pos.z_;
		float y1_test = h_factor * render_list_->transparent_tri_list_[i].vertex_list[1].pos.z_;
		float y2_test = h_factor * render_list_->transparent_tri_list_[i].vertex_list[2].pos.z_;
		//对于上下截面只考虑完全接受或者完全拒绝，即所有点都在视锥体上边或者都在视锥体下边
		if ((render_list_->transparent_tri_list_[i].vertex_list[0].pos.y_ > y0_test
			&& render_list_->transparent_tri_list_[i].vertex_list[1].pos.y_ > y1_test
			&& render_list_->transparent_tri_list_[i].vertex_list[2].pos.y_ > y2_test)
			|| (render_list_->transparent_tri_list_[i].vertex_list[0].pos.y_ < -y0_test
				&& render_list_->transparent_tri_list_[i].vertex_list[1].pos.y_ < -y1_test
				&& render_list_->transparent_tri_list_[i].vertex_list[2].pos.y_ < -y2_test))
		{
			render_list_->transparent_active_tri_[i] = false;
		...
		//对每一个顶点比较z值与远近平面的关系，计数并填入索引
		if ((render_list_->transparent_tri_list_[i].vertex_list[0].pos.z_ < near_clip
			&& render_list_->transparent_tri_list_[i].vertex_list[0].pos.z_ > far_clip)) {
			verts_inside[0] = true;
			++num_verts_inside;
		}
		else if (render_list_->transparent_tri_list_[i].vertex_list[0].pos.z_ > near_clip)
		{
			verts_outside[0] = true;
			++num_verts_outside_near_clip;
		}

		if ((render_list_->transparent_tri_list_[i].vertex_list[1].pos.z_ < near_clip
			&& render_list_->transparent_tri_list_[i].vertex_list[1].pos.z_ > far_clip)) {
			verts_inside[1] = true;
			++num_verts_inside;
		}
		else if (render_list_->transparent_tri_list_[i].vertex_list[1].pos.z_ > near_clip)
		{
			verts_outside[1] = true;
			++num_verts_outside_near_clip;
		}

		if ((render_list_->transparent_tri_list_[i].vertex_list[2].pos.z_ < near_clip
			&& render_list_->transparent_tri_list_[i].vertex_list[2].pos.z_ > far_clip)) {
			verts_inside[2] = true;
			++num_verts_inside;
		}
		else if (render_list_->transparent_tri_list_[i].vertex_list[2].pos.z_ > near_clip)
		{
			verts_outside[2] = true;
			++num_verts_outside_near_clip;
		}
		...
		if (num_verts_outside_near_clip >= 1) {
			...
			if (num_verts_inside == 1)
			{
				...
				//根据线段参数方程p = v0_idx + v * t (0 < t < 1)
				float diff = (near_clip - render_list_->transparent_tri_list_[i].vertex_list[v0_idx].pos.z_);
				KZMath::KZVector4D<float> vec1(render_list_->transparent_tri_list_[i].vertex_list[v1_idx].pos - render_list_->transparent_tri_list_[i].vertex_list[v0_idx].pos);
				float t1 = diff / vec1.z_;

				//根据参数t计算出交点的pos,代替原来顶点的pos
				render_list_->transparent_tri_list_[i].vertex_list[v1_idx].pos.x_ = render_list_->transparent_tri_list_[i].vertex_list[v0_idx].pos.x_ + t1 * vec1.x_;
				render_list_->transparent_tri_list_[i].vertex_list[v1_idx].pos.y_ = render_list_->transparent_tri_list_[i].vertex_list[v0_idx].pos.y_ + t1 * vec1.y_;
				render_list_->transparent_tri_list_[i].vertex_list[v1_idx].pos.z_ = near_clip;

				//根据参数t计算出交点的color,代替原来顶点的color
				render_list_->transparent_tri_list_[i].vertex_list[v1_idx].color.r_ = (unsigned char)(render_list_->transparent_tri_list_[i].vertex_list[v0_idx].color.r_
					+ t1 * (render_list_->transparent_tri_list_[i].vertex_list[v1_idx].color.r_ - render_list_->transparent_tri_list_[i].vertex_list[v0_idx].color.r_));

				render_list_->transparent_tri_list_[i].vertex_list[v1_idx].color.g_ = (unsigned char)(render_list_->transparent_tri_list_[i].vertex_list[v0_idx].color.g_
					+ t1 * (render_list_->transparent_tri_list_[i].vertex_list[v1_idx].color.g_ - render_list_->transparent_tri_list_[i].vertex_list[v0_idx].color.g_));

				render_list_->transparent_tri_list_[i].vertex_list[v1_idx].color.b_ = (unsigned char)(render_list_->transparent_tri_list_[i].vertex_list[v0_idx].color.b_
					+ t1 * (render_list_->transparent_tri_list_[i].vertex_list[v1_idx].color.b_ - render_list_->transparent_tri_list_[i].vertex_list[v0_idx].color.b_));

				//根据线段参数方程p = v0 + v * t (0 < t < 1)
				KZMath::KZVector4D<float> vec2(render_list_->transparent_tri_list_[i].vertex_list[v2_idx].pos - render_list_->transparent_tri_list_[i].vertex_list[v0_idx].pos);
				float t2 = diff / vec2.z_;
				//根据参数t计算出交点pos，代替原来pos
				render_list_->transparent_tri_list_[i].vertex_list[v2_idx].pos.x_ = render_list_->transparent_tri_list_[i].vertex_list[v0_idx].pos.x_ + t2 * vec2.x_;
				render_list_->transparent_tri_list_[i].vertex_list[v2_idx].pos.y_ = render_list_->transparent_tri_list_[i].vertex_list[v0_idx].pos.y_ + t2 * vec2.y_;
				render_list_->transparent_tri_list_[i].vertex_list[v2_idx].pos.z_ = near_clip;

				//根据参数t计算出交点的color,代替原来顶点的color
				render_list_->transparent_tri_list_[i].vertex_list[v2_idx].color.r_ = (unsigned char)(render_list_->transparent_tri_list_[i].vertex_list[v0_idx].color.r_
					+ t2 * (render_list_->transparent_tri_list_[i].vertex_list[v2_idx].color.r_ - render_list_->transparent_tri_list_[i].vertex_list[v0_idx].color.r_));

				render_list_->transparent_tri_list_[i].vertex_list[v2_idx].color.g_ = (unsigned char)(render_list_->transparent_tri_list_[i].vertex_list[v0_idx].color.g_
					+ t2 * (render_list_->transparent_tri_list_[i].vertex_list[v2_idx].color.g_ - render_list_->transparent_tri_list_[i].vertex_list[v0_idx].color.g_));

				render_list_->transparent_tri_list_[i].vertex_list[v2_idx].color.b_ = (unsigned char)(render_list_->transparent_tri_list_[i].vertex_list[v0_idx].color.b_
					+ t2 * (render_list_->transparent_tri_list_[i].vertex_list[v2_idx].color.b_ - render_list_->transparent_tri_list_[i].vertex_list[v0_idx].color.b_));
				...
				if (render_list_->transparent_tri_list_[i].material != -1 && mat_vec_[render_list_->transparent_tri_list_[i].material].has_texture_) {
					...
					render_list_->transparent_tri_list_[i].vertex_list[v1_idx].uv.x_ = render_list_->transparent_tri_list_[i].vertex_list[v0_idx].uv.x_
						+ t1 * (render_list_->transparent_tri_list_[i].vertex_list[v1_idx].uv.x_ - render_list_->transparent_tri_list_[i].vertex_list[v0_idx].uv.x_);

					render_list_->transparent_tri_list_[i].vertex_list[v1_idx].uv.y_ = render_list_->transparent_tri_list_[i].vertex_list[v0_idx].uv.y_
						+ t1 * (render_list_->transparent_tri_list_[i].vertex_list[v1_idx].uv.y_ - render_list_->transparent_tri_list_[i].vertex_list[v0_idx].uv.y_);

					render_list_->transparent_tri_list_[i].vertex_list[v2_idx].uv.x_ = render_list_->transparent_tri_list_[i].vertex_list[v0_idx].uv.x_
						+ t2 * (render_list_->transparent_tri_list_[i].vertex_list[v2_idx].uv.x_ - render_list_->transparent_tri_list_[i].vertex_list[v0_idx].uv.x_);

					render_list_->transparent_tri_list_[i].vertex_list[v2_idx].uv.y_ = render_list_->transparent_tri_list_[i].vertex_list[v0_idx].uv.y_
						+ t2 * (render_list_->transparent_tri_list_[i].vertex_list[v2_idx].uv.y_ - render_list_->transparent_tri_list_[i].vertex_list[v0_idx].uv.y_);
				}
			}
			//当有两个顶点在视锥体里面,需要重新分割一个三角形
			else if (num_verts_inside == 2 && num_verts_outside_near_clip == 1)
			{
				...
				Triangle new_tri(render_list_->transparent_tri_list_[i]);
				...
				KZMath::KZVector4D<float> ori_v0_pos = render_list_->transparent_tri_list_[i].vertex_list[v0_idx].pos;
				//根据参数t计算出交点的pos,代替外部顶点的pos
				render_list_->transparent_tri_list_[i].vertex_list[v0_idx].pos.x_ = render_list_->transparent_tri_list_[i].vertex_list[v0_idx].pos.x_ + t1 * vec1.x_;
				render_list_->transparent_tri_list_[i].vertex_list[v0_idx].pos.y_ = render_list_->transparent_tri_list_[i].vertex_list[v0_idx].pos.y_ + t1 * vec1.y_;
				render_list_->transparent_tri_list_[i].vertex_list[v0_idx].pos.z_ = near_clip;

				//根据参数t计算出交点的color,代替外部顶点的color
				float ori_v0_r = render_list_->transparent_tri_list_[i].vertex_list[v0_idx].color.r_;
				float ori_v0_g = render_list_->transparent_tri_list_[i].vertex_list[v0_idx].color.g_;
				float ori_v0_b = render_list_->transparent_tri_list_[i].vertex_list[v0_idx].color.b_;
				render_list_->transparent_tri_list_[i].vertex_list[v0_idx].color.r_ = (unsigned char)(render_list_->transparent_tri_list_[i].vertex_list[v0_idx].color.r_
					+ t1 * (render_list_->transparent_tri_list_[i].vertex_list[v1_idx].color.r_ - render_list_->transparent_tri_list_[i].vertex_list[v0_idx].color.r_));
				render_list_->transparent_tri_list_[i].vertex_list[v0_idx].color.g_ = (unsigned char)(render_list_->transparent_tri_list_[i].vertex_list[v0_idx].color.g_
					+ t1 * (render_list_->transparent_tri_list_[i].vertex_list[v1_idx].color.g_ - render_list_->transparent_tri_list_[i].vertex_list[v0_idx].color.g_));
				render_list_->transparent_tri_list_[i].vertex_list[v0_idx].color.b_ = (unsigned char)(render_list_->transparent_tri_list_[i].vertex_list[v0_idx].color.b_
					+ t1 * (render_list_->transparent_tri_list_[i].vertex_list[v1_idx].color.b_ - render_list_->transparent_tri_list_[i].vertex_list[v0_idx].color.b_));

				//默认法线不变，点法线插值计算不值得特别精确用原本点法线代替即可,并且已经计算了光照

				float ori_v0_u = render_list_->transparent_tri_list_[i].vertex_list[v0_idx].uv.x_;
				float ori_v0_v = render_list_->transparent_tri_list_[i].vertex_list[v0_idx].uv.y_;
				//根据是否有贴图计算交点的uv,代替原来的uv
				if (render_list_->transparent_tri_list_[i].material != -1 && mat_vec_[render_list_->transparent_tri_list_[i].material].has_texture_)
				{
					render_list_->transparent_tri_list_[i].vertex_list[v0_idx].uv.x_ = render_list_->transparent_tri_list_[i].vertex_list[v0_idx].uv.x_
						+ t1 * (render_list_->transparent_tri_list_[i].vertex_list[v1_idx].uv.x_ - render_list_->transparent_tri_list_[i].vertex_list[v0_idx].uv.x_);

					render_list_->transparent_tri_list_[i].vertex_list[v0_idx].uv.y_ = render_list_->transparent_tri_list_[i].vertex_list[v0_idx].uv.y_
						+ t1 * (render_list_->transparent_tri_list_[i].vertex_list[v1_idx].uv.y_ - render_list_->transparent_tri_list_[i].vertex_list[v0_idx].uv.y_);
				}

				//新三角形的保留v2不变
				new_tri.vertex_list[v1_idx] = render_list_->transparent_tri_list_[i].vertex_list[v0_idx];

				//根据线段参数方程p = v0 + v * t (0 < t < 1)
				//diff = (near_clip - render_list_->tri_list_[i].vertex_list[v0_idx].pos.z_);
				KZMath::KZVector4D<float> vec2(render_list_->transparent_tri_list_[i].vertex_list[v2_idx].pos - ori_v0_pos);
				float t2 = diff / vec2.z_;
				...
				//根据参数t计算出交点的color
				new_tri.vertex_list[v0_idx].color.r_ = (unsigned char)(ori_v0_r +
					t2 * (render_list_->transparent_tri_list_[i].vertex_list[v2_idx].color.r_ - ori_v0_r));
				new_tri.vertex_list[v0_idx].color.g_ = (unsigned char)(ori_v0_g +
					t2 * (render_list_->transparent_tri_list_[i].vertex_list[v2_idx].color.g_ - ori_v0_g));
				new_tri.vertex_list[v0_idx].color.b_ = (unsigned char)(ori_v0_b +
					t2 * (render_list_->transparent_tri_list_[i].vertex_list[v2_idx].color.b_ - ori_v0_b));

				//默认法线不变，点法线插值计算不值得特别精确用原本点法线代替即可,并且已经计算了光照
				new_tri.vertex_list[1].normal = render_list_->transparent_tri_list_[i].vertex_list[v2_idx].normal;

				//根据是否有贴图计算交点的uv,代替原来的uv
				if (render_list_->transparent_tri_list_[i].material != -1 && mat_vec_[render_list_->transparent_tri_list_[i].material].has_texture_)
				{
					new_tri.vertex_list[v0_idx].uv.x_ = ori_v0_u
						+ t2 * (render_list_->transparent_tri_list_[i].vertex_list[v2_idx].uv.x_ - ori_v0_u);

					new_tri.vertex_list[v0_idx].uv.y_ = ori_v0_v
						+ t2 * (render_list_->transparent_tri_list_[i].vertex_list[v2_idx].uv.y_ - ori_v0_v);
				}

				//将三角形插入渲染队列
				++transparent_tri_num_;
				if (tri_num_ > render_list_->transparent_tri_list_.size()) {
					render_list_->transparent_tri_list_.resize(transparent_tri_num_ * 2);
					render_list_->transparent_active_tri_.resize(transparent_tri_num_ * 2);
				}
				render_list_->transparent_tri_list_[transparent_tri_num_ - 1] = new_tri;
				render_list_->transparent_active_tri_[transparent_tri_num_ - 1] = true;
...
void KZPipeLine::getSceneAABB(bool include_transparent) {
	...
	for (uint32_t i = 0; i < object_num_; ++i) {
		object_vec_[i].GetObjectAABB(aabb_min_, aabb_max_);
	}
	if (include_transparent) {
		for (uint32_t i = 0; i < transparent_object_num_; ++i) {
			transparent_object_vec_[i].GetObjectAABB(aabb_min_, aabb_max_);
		}
	}
}
